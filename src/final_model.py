...
def train_final_and_score_test(
    ...
    df_all_train = features_df[features_df["test"] == False].copy()
    ...
    # 2) score test only
    df_test = features_df[features_df["test"] == True].copy()
...
# usage:
# model, test_pred_df = train_final_and_score_test(features_df, cfg, batch_size=64, num_workers=8)
# threshold = 0.90  # from your df_summary decision
# test_pred_df["pred"] = (test_pred_df["prob"] >= threshold).astype(int)
# test_pred_df.head()
# torch.save(model.state_dict(), "data/final_model_state_dict.pt")
